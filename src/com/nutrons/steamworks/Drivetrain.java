...
public class Drivetrain implements Subsystem {
  ...
  /**
   * Drives the robot for a desired amout of time, for testing purposes.
   * @param time desired amount of time in millseconds.
   * @return command for autonomous.
   */
  public Command driveTimeAction(long time) {
    Flowable<Double> move = toFlow(() -> 1.0);
    return Command.fromSubscription(
        () -> combineDisposable(move.map(x -> Events.power(x)).subscribe(leftDrive),
            move.map(x -> Events.power(-x)).subscribe(rightDrive)))
        .killAfter(time, TimeUnit.MILLISECONDS);
  }
