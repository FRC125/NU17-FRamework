...
public class Shooter implements Subsystem {
  ...
  public void registerSubscriptions() {
    ...
    //Consumer<Double> shooter_setpoint = new WpiSmartDashboard().getTextFieldDouble("shooter-setpoint");
    ...
    shooterButton.map(FlowOperators::printId).map(x -> x ? Events.combine(Events.mode(ControlMode.LOOP_SPEED),
        Events.setpoint( prefs.getDouble("shooter_setpoint", 3250.0))) : Events.combine(Events.setpoint(0), Events.power(0)))
